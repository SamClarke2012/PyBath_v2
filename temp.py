...
if __name__ == "__main__":
    ...
    def temperature_to_joules(temp_obj, temp_amb, mass_kg, heat_capacity):
        ...
        return float((heat_capacity * (mass_kg*1000) * (temp_obj - temp_amb)))
    ...
    while window_thread.is_alive():
        ...
        if shared_memory['start'] and shared_memory['target'] is not None:
            ...
            if shared_memory['start'] and window_thread.is_alive():
                cycleStart = time.time()
                current_energy = temperature_to_joules(shared_memory['bath_temp'], 
                                                       shared_memory['env_temp'], m, h)
                ...
                if element_time > 0.0: get_temp(send=True, on_time=int(element_time*1000))
